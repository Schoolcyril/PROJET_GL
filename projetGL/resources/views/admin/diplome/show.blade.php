@extends('layouts.app')

@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">Details du diplome {{ $diplome->id }}</div>
                    <div class="card-body">

                        <a href="{{ url('/admin/diplome') }}" title="Back"><button class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</button></a>
                        <a href="{{ url('/admin/diplome/edit/'.$diplome->id) }}" title="Edit Apprenant"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

                        <form method="POST" action="{{ url('admin/diplome/delete/'. $diplome->id) }}" accept-charset="UTF-8" style="display:inline">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                            <button type="submit" class="btn btn-danger btn-sm" title="Delete Apprenant" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
                        </form>
                        <br/>
                        <br/>

                        <div class="table-responsive">
                            <table class="table">
                                <tr>
                                    <th>Année d'obtention</th>
                                    <td>{{$diplome->année_obtention}}</td>
                                </tr>
                                <tr>
                                    <th>Apprenant</th>
                                    <td>{{$diplome->nom}}</td>
                                </tr>
                                <tr>
                                    <th>Mention</th>
                                    <td>{{$diplome->mention}}</td>
                                </tr>
                            </table>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
